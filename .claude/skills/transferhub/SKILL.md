...
**Attribution:** `src/data/imageCredits.json` maps a player-slug → {author, license, url} for any CC-BY / CC-BY-SA drop-in. `/credits` page renders them; a small "© author" caption shows on those cards. PD/CC0 need no entry. Pixabay + our own art need no attribution.
